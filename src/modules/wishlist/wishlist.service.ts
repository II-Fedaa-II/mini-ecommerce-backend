...
export class WishlistService {
  ...
  async getWishlist(userId: string): Promise<ProductResponseDto[]> {
    ...
    const products = await this.productsService.findManyByIds(productIds.map((id) => id.toString()));
    return products.map(ProductResponseDto.fromDocument);
  }

  async addItem(userId: string, productId: string): Promise<ProductResponseDto[]> {
    await this.productsService.findDocumentOrThrow(productId);
    await this.usersService.addToWishlist(userId, productId);
    return this.getWishlist(userId);
  }

  async removeItem(userId: string, productId: string): Promise<ProductResponseDto[]> {
