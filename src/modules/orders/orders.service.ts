...
import { EmptyCartException, InsufficientStockException, OrderNotFoundException } from '../../common/exceptions/domain.exceptions';
...
export class OrdersService {
  ...
  async checkout(userId: string): Promise<OrderResponseDto> {
    ...
    for (const item of cartItems) {
      ...
      if (!product || product.stock < item.quantity) {
        throw new InsufficientStockException(item.productId.toString(), item.quantity, product?.stock ?? 0);
      }
    }
    ...
    const order = await this.ordersRepository.create({ userId, items: orderLines, total });

    await this.productsService.decrementManyStock(
      orderLines.map((line) => ({ productId: line.productId.toString(), amount: line.quantity })),
    );
  ...
  async getById(userId: string, orderId: string): Promise<OrderResponseDto> {
    ...
    if (!order || order.userId.toString() !== userId) throw new OrderNotFoundException(orderId);
    return OrderResponseDto.fromDocument(order);
  }
}
